...
//const  {customerTestDate} = require ('../ApiUtilis_copy/test_data_fixture')
import { customerTestDate } from "../ApiUtilis_copy/test_data_fixture"; 
...
// If your tsconfig.json has "resolveJsonModule": true, use:
import dataSet from "../ApiUtilis_copy/placeOrderTestDate.json";

// Otherwise, use:
// const dataSet = require("../ApiUtilis/placeOrderTestDate.json");
...
test.describe('@WEB Store credetianls after registration', () => {
   ...
   // let emailAddress = dataSet.emailAddress;
    ...
    test('@WEB Registration step,and verify successfully login msg ', async ({ page }: {page:Page}) => {
    ...
    test(`@WEB Check the displayed itesm on the page with user , IT SHOULD SKIP due to fail previous TEST `, async ({ page }:{page:Page}) => {
...
customerTestDate(`Check the displayed items with derived data set from JSON as fixture `, async ({ page,storateDataFixture }:{page:Page,storateDataFixture:any}) => {
        const poManager = new POManager(page);
        let productName = 'ZARA COAT 3';
        const products = page.locator('.card-body');
        const loginPage = poManager.getLoginPage();
